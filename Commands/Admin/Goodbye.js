...
module.exports = {
  ...
  usage: [
    ...
    "Set channel for the goodbye message```{prefix}goodbye set #channel```",
    "Set up a custom goodbye message```{prefix}goodbye custom <text>```",
    "Disable the goodbye message```{prefix}goodbye disable```",
    "Disable the goodbye message```{prefix}goodbye disable```",
    "Test the goodbye message```{prefix}goodbye test```",
  ...
  async execute(client, message, args, data) {
    try {
      ...
      if (args[0].toLowerCase() === "test") {
        ...
        if (
          !data.guild.addons.goodbye.enabled ||
          data.guild.addons.goodbye.channel.trim() === ""
        ) {
          return message.channel.send(
            "Goodbye messages are currently disabled."
          );
        }
        ...
        let goodbyeMsg =
          data.guild.addons.goodbye.message === null ||
          data.guild.addons.goodbye.message === "" ||
          data.guild.addons.goodbye.message === " "
            ? "{user.ping} has left the server!"
            : data.guild.addons.goodbye.message; // Get the custom message or use the preset one
